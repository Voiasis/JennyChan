            ...
            if (event.getMessage().getMentionedUsers().get(0).getId().equals("472899069136601099")) {
                if (event.getAuthor().isBot()) {//
                    ...
                    if (event.getMessage().getContentRaw().contains("472899069136601099")) {
                        event.getMessage().reply("https://c.tenor.com/2QwIi_u-BBMAAAAi/ping.gif").queue();
                    } else {
                        event.getMessage().reply("https://www.youtube.com/watch?v=FmO5IPwUuv8").queue();
                    }
                }
            }
        } catch(Exception ex) {
